...
export default function PaymentSuccess() {
    ...
    useEffect(() => {
        async function fetchSuccessPaypalPayment() {
            if (paymentId && PayerID && token) {
                try {
                    const resMessage = await successPaypalPayment(token, paymentId, PayerID);
    ...
    return (
        <div>
            {message === SUCCESS_MESSAGE && <div>Thank you! Your payment was successful.</div>}
            {message === ERROR_MESSAGE && <div>Sorry, there was an error with your payment.</div>}
